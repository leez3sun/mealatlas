...
3. 穷举当前三餐组合，选择最接近每日能量目标且兼顾蛋白、纤维、钠与菜系多样性的组合；
4. 将目标能量折算为可见份量系数，同步缩放卡片、详情与全天营养；
5. 改良只替换当前食谱的显式营养版本。

当前 72 道站内菜谱覆盖全部 71 个菜系条目，可以直接打开详情并进入配餐和营养计算（北京菜包含两道示例）。每道代表菜都有独立配料与步骤，重复字段由类型化构建器统一生成；外部链接用于继续核验，不复制作者正文。未来数据量足够后，可把评分器替换为带硬约束的整数/目标规划。

## Data honesty

当前 72 道食谱是交互原型数据，营养值均标为 `estimated`。这表示它们是按一人份克重和食材类别汇总的工程估算，并非实验室检测。生产版计划接入：
...
13 道精选菜谱使用独立的项目自有成品摄影，不再跨菜名复用图片；新增的 59 道目录菜谱使用唯一的编辑式封面，并明确标注“成品摄影待补充”，避免用另一道菜的照片冒充。所有图片均以菜谱 ID 绑定，自动测试会检查一菜一图与资源文件存在性。
